...
//       const productNames = orderData.OrderProducts.map(op => op.Product?.product_description?.[0]?.name || op.name).join(', ');
...
//         name: op.Product?.product_description?.[0]?.name || op.name,
//         model: op.model,
...
exports.getAllOrders = async (req, res) => {
  try {
    console.log('getAllOrders called with customer:', req.customer);
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const offset = (page - 1) * limit;
    const status = req.query.status; // Get status filter from query params
    ...
    const formattedOrders = orders.rows.map(order => {
      ...
      return {
        ...
        courier_id: orderData.courier_id,
        courier_name: orderData.courier_name,
        total_courier_charges: orderData.total_courier_charges,
        product_names: productNames,
        product_images: productImages,
        comment: orderData.comment
      };
...
// Track order by AWB number using ClickPost API
exports.trackOrder = async (req, res) => {
  try {
    const { awbno } = req.params;
    ...
    const order = await Order.findOne({
      ...
      attributes: ['order_id', 'awbno', 'courier_id', 'courier_name']
    });
    ...
    // Get courier partner ID from order or use default
    const courierId = order.courier_id || 8; // Default to DTDC if not specified
    
    // Import ClickPost service
    const ClickPostService = require('../../services/clickpost.service');
    
    // Call ClickPost tracking API
    const trackingResult = await ClickPostService.trackOrder(awbno, courierId);
    ...
    if (!trackingResult.success) {
      return res.status(500).json({
        meta: {
          status: 500,
          success: false,
          message: trackingResult.error || 'Could not track order'
        },
        result: null
      });
    }

    // Return the ClickPost response in the requested format
    res.status(200).json({
      meta: {
        status: 200,
        success: true,
