  ...
  try {
    ...
    try {
      ...
      if (error.name === 'JsonWebTokenError') {
        console.log('Invalid token format in optional auth, continuing without authentication');
      } else if (error.name === 'TokenExpiredError') {
        console.log('Expired token in optional auth, continuing without authentication');
      } else {
        console.log('Token error in optional auth, continuing without authentication:', error.message);
      }
    }
  ...
  try {
    ...
    try {
      ...
      if (error.name === 'JsonWebTokenError') {
        ...
        console.error('JWT Not Before:', error.message);
        return res.status(401).json({
          success: false,
